...
import {
  getQualityRecords, getUploadLog, deleteUpload,
  type QualityRecord, type UploadLogEntry,
} from '../../../api/dashboard'
import { saveQCResults, getProjects } from '../../../api/projects'
...
export function QualityReportTab({ projectId }: Props) {
  ...
  const { filters, filtered, activeCount, setFilter, clearFilters, uniqueVals } = useFilters(
    ...
    ['interviewer_id', 'region', 'approval_status', 'duration_flag', 'straight_lining'],
  )
  ...
  const filterDefs: FilterDef[] = [
    ...
    { key: 'straight_lining', label: 'Straightlining', type: 'select', options: uniqueVals['straight_lining'] ?? [] },
  ...
  const approved = filtered.filter(r => (r as QualityRecord).approval_status === 'Approved').length
  const pending = filtered.filter(r => (r as QualityRecord).approval_status === 'Pending').length
  const cancelled = filtered.filter(r => (r as QualityRecord).approval_status === 'Cancelled').length
  const flagged = filtered.filter(r => (r as QualityRecord).duration_flag === 'Flag').length
  ...
  const pieColors = [C.accent, '#3B5A9A', C.critical]

  const byItv: Record<string, { duration_flag: number; straight_lining: number }> = {}
  filtered.forEach(r => {
    const rec = r as QualityRecord
    const id = rec.interviewer_id ?? 'Unknown'
    if (!byItv[id]) byItv[id] = { duration_flag: 0, straight_lining: 0 }
    if (rec.duration_flag === 'Flag') byItv[id].duration_flag++
    if (rec.straight_lining === 'Flag') byItv[id].straight_lining++
  })
  ...
  filtered.forEach(r => {
    const rec = r as QualityRecord
    if (rec.interview_date) byDay[rec.interview_date] = (byDay[rec.interview_date] ?? 0) + 1
  })
  const dayData = Object.entries(byDay).sort(([a], [b]) => a.localeCompare(b))
    .map(([date, count]) => ({ date, count }))

  const canUpload = authUser && UPLOAD_ROLES.has(authUser.role)
  const qcReady = !!fileId
  const qrLogs = uploadLog.filter((u: UploadLogEntry) => u.report_type === 'quality_report')

  const handleCsvDownload = () => exportCSV(filtered as Record<string, unknown>[], `quality_report_${projectId}.csv`)
  const handleExcelDownload = async () => {
    setIsExporting(true)
    try {
      await exportExcel(projectId, 'quality_report', filtered as Record<string, unknown>[], {
        type: 'bar', data: errorData, x: 'name', y: 'duration_flag', title: 'Errors by Interviewer',
      }, token, `quality_report_${projectId}.xlsx`)
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="card p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-3 items-end">
            <div>
              ...
              <select value={selectedProjectId ?? projectId} onChange={e => setSelectedProjectId(Number(e.target.value))}
            ...
            <button onClick={() => saveMut.mutate()} disabled={saveMut.isPending} className="btn-primary flex items-center gap-2">
              {saveMut.isPending ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />} Save to DB
            </button>
        ...
        <div className="card p-4">
          ...
          <table className="w-full text-xs">
            <thead><tr className="border-b border-line text-muted">
              {['File', 'Wave', 'Rows', 'By', 'Date', ''].map(h => <th key={h} className="text-left py-1 pr-3">{h}</th>)}
            </tr></thead>
            <tbody>
              {qrLogs.map((u: UploadLogEntry) => (
      ...
      {!isLoading && records.length === 0 && (
        <div className="card p-8 text-center text-muted text-sm">No quality records yet.</div>
      )}
      ...
      {records.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {[
              { label: 'Total', value: total, color: 'text-tx' },
              { label: 'Approved', value: approved, color: 'text-accent' },
              { label: 'Pending', value: pending, color: 'text-tx' },
              { label: 'Cancelled', value: cancelled, color: 'text-critical' },
              { label: 'Duration Flags', value: flagged, color: 'text-critical' },
            ].map(k => (
              ...
              <div className="card p-4">
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={errorData} margin={{ left: -20 }}>
                    ...
                    <Bar dataKey="straight_lining" name="Straight Lining" fill="#3B5A9A" radius={[2, 2, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
          ...
          <div className="card p-4">
            ...
            <div className="overflow-x-auto max-h-80">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {(filtered as QualityRecord[]).map(r => (
                    <tr key={r.id} className="border-b border-line/30 hover:bg-surface2/50">
                      ...
                      <td className="py-1 pr-3">
                        {r.duration_flag === 'Flag' ? <span className="badge-critical">Flag</span> : <span className="text-muted">{r.duration_flag ?? '—'}</span>}
                      </td>
                      <td className="py-1 pr-3">
                        {r.straight_lining === 'Flag' ? <span className="badge-warning">Flag</span> : <span className="text-muted">{r.straight_lining ?? '—'}</span>}
                      </td>
                      <td className="py-1 pr-3">
                        <span className={r.approval_status === 'Approved' ? 'text-accent' : r.approval_status === 'Cancelled' ? 'text-critical' : 'text-tx'}>
                          {r.approval_status ?? '—'}
                        </span>
                      </td>
